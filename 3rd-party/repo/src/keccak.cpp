...
void keccak(const void* in_ptr, int inlen, uint8_t* md, int mdlen, int rounds)
{
	const uint8_t* in = reinterpret_cast<const uint8_t*>(in_ptr);
	uint64_t st[25];

	const int rsiz = (sizeof(st) == mdlen) ? 136 : (200 - 2 * mdlen);
	...
	for (; inlen >= rsiz; inlen -= rsiz, in += rsiz) {
		for (int i = 0; i < rsizw; i++) {
			uint64_t t;
			memcpy(&t, reinterpret_cast<const uint64_t*>(in) + i, sizeof(uint64_t));
			st[i] ^= t;
		}
		keccakf(st, rounds);
	}
